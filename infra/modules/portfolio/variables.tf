...
variable "certificate_arn" {
  description = "Optional ACM certificate ARN for HTTPS. Used when domain_name is set."
  type        = string
  default     = ""
}

variable "backend_image" {
  description = "Backend ECR image URI (tag or digest)"
  type        = string
}

variable "frontend_image" {
  description = "Frontend ECR image URI (tag or digest)"
  type        = string
}
